...
feature -- parsing
	...
	evt_to_cmd (evt : TUPLE[name: STRING; args: ARRAY[ETF_EVT_ARG]]) : ETF_COMMAND_INTERFACE
		...
		do
			...
			if cmd_name ~ "new" then 
				 if attached {ETF_STR_ARG} args[1] as a_id and then TRUE then 
					 create {ETF_NEW} Result.make ("new", [a_id.value], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 

			elseif cmd_name ~ "deposit" then 
				 if attached {ETF_STR_ARG} args[1] as a_id and then TRUE and then attached {ETF_VALUE_ARG} args[2] as a_value and then TRUE then 
					 create {ETF_DEPOSIT} Result.make ("deposit", [a_id.value , create {VALUE}.make_from_string (a_value.src_out)], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
			...
			elseif cmd_name ~ "withdraw" then 
				 if attached {ETF_STR_ARG} args[1] as a_id and then TRUE and then attached {ETF_VALUE_ARG} args[2] as a_value and then TRUE then 
					 create {ETF_WITHDRAW} Result.make ("withdraw", [a_id.value , create {VALUE}.make_from_string (a_value.src_out)], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 

			elseif cmd_name ~ "transfer" then 
				 if attached {ETF_STR_ARG} args[1] as id1 and then TRUE and then attached {ETF_STR_ARG} args[2] as id2 and then TRUE and then attached {ETF_VALUE_ARG} args[3] as a_value and then TRUE then 
					 create {ETF_TRANSFER} Result.make ("transfer", [id1.value , id2.value , create {VALUE}.make_from_string (a_value.src_out)], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
			else 
	...
	do
		...
		loop
			...
			if cmd_name ~ "new" then 
				if NOT( ( args.count = 1 ) AND THEN attached {ETF_STR_ARG} args[1] as a_id and then TRUE) then 
			...
			elseif cmd_name ~ "deposit" then 
				if NOT( ( args.count = 2 ) AND THEN attached {ETF_STR_ARG} args[1] as a_id and then TRUE and then attached {ETF_VALUE_ARG} args[2] as a_value and then TRUE) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
							"deposit(a_id: ID = STRING ; a_value: VALUE)")
				end
			...
			elseif cmd_name ~ "withdraw" then 
				if NOT( ( args.count = 2 ) AND THEN attached {ETF_STR_ARG} args[1] as a_id and then TRUE and then attached {ETF_VALUE_ARG} args[2] as a_value and then TRUE) then 
					...
					Result.append (evt_out_str + " does not conform to declaration " +
							"withdraw(a_id: ID = STRING ; a_value: VALUE)")
				end

			elseif cmd_name ~ "transfer" then 
				if NOT( ( args.count = 3 ) AND THEN attached {ETF_STR_ARG} args[1] as id1 and then TRUE and then attached {ETF_STR_ARG} args[2] as id2 and then TRUE and then attached {ETF_VALUE_ARG} args[3] as a_value and then TRUE) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
							"transfer(id1: ID = STRING ; id2: ID = STRING ; a_value: VALUE)")
				end
			else
				if NOT Result.is_empty then
					Result.append ("%N")
